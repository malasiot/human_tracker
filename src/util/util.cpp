...
void get_bone_geometry_recursive(const std::string &bname, const std::map<std::string, MHX2Bone> &skeleton, const std::map<std::string, Vector3f> &joints,
                                 std::vector<std::pair<Vector3f, Vector3f>> &coords) {
    auto it = joints.find(bname) ;
    assert( it != joints.end() ) ;

    const Vector3f &coords0 = (*it).second ;

    // iterate children

    for( const auto &bp: skeleton ) {
        if ( bp.second.parent_ == bname ) {
            auto it = joints.find(bp.first) ;
            assert( it != joints.end() ) ;

            const auto &coords1 = it->second ;
            coords.emplace_back(std::make_pair(coords0, coords1)) ;
            get_bone_geometry_recursive(it->first, skeleton, joints, coords) ;
        }
    }
}

void get_bone_geometry(const std::map<std::string, MHX2Bone> &skeleton, const std::map<std::string, Vector3f> &joints,
                       std::vector<std::pair<Vector3f, Vector3f>> &coords) {

    // find root bone

    string root ;
    for( const auto &jp : skeleton ) {
        if ( jp.second.parent_.empty() ) {
            root = jp.first ;
            break ;
        }
    }

    // recursively visit bones
    get_bone_geometry_recursive(root, skeleton, joints, coords) ;
}

void skeleton_to_obj(const std::map<std::string, MHX2Bone> &skeleton, const std::map<std::string, Vector3f> &joints,
